...
async fn setup() -> (BanksClient, Keypair, Hash) {
    let mut program_test = ProgramTest::new(
        "example_program_program",
        example_program_api::ID,
        processor!(example_program_program::process_instruction),
    );
...
async fn run_test() {
    ...
    assert_eq!(counter_account.owner, example_program_api::ID);
    ...
    // Submit sub transaction.
    let ix = sub(payer.pubkey(), 12);
    let tx = Transaction::new_signed_with_payer(&[ix], Some(&payer.pubkey()), &[&payer], blockhash);
    let res = banks.process_transaction(tx).await;
    assert!(res.is_ok());
